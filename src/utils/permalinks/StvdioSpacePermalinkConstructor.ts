 ...
 */
export default class StvdioSpacePermalinkConstructor extends PermalinkConstructor {
    private stvdioSpaceUrl: string;

    constructor(stvdioSpaceUrl: string) {
        super();
        this.stvdioSpaceUrl = stvdioSpaceUrl;

        if (!this.stvdioSpaceUrl.startsWith("http:") && !this.stvdioSpaceUrl.startsWith("https:")) {
            throw new Error("STVDIO Space prefix URL does not appear to be an HTTP(S) URL");
        }
    }

    // --DTM-- New constructor that defaults to the STVDIO Space domain
    // constructor() {
    //     super();
    //     this.stvdioSpaceUrl = "https://space.stvd.io";
    // }

    forEvent(roomId: string, eventId: string, serverCandidates: string[]): string {
        return `${this.stvdioSpaceUrl}/#/room/${roomId}/${eventId}${this.encodeServerCandidates(serverCandidates)}`;
    }

    forRoom(roomIdOrAlias: string, serverCandidates?: string[]): string {
        return `${this.stvdioSpaceUrl}/#/room/${roomIdOrAlias}${this.encodeServerCandidates(serverCandidates)}`;
    }

    forUser(userId: string): string {
        return `${this.stvdioSpaceUrl}/#/user/${userId}`;
    }

    forGroup(groupId: string): string {
        return `${this.stvdioSpaceUrl}/#/group/${groupId}`;
    }
    ...
    isPermalinkHost(testHost: string): boolean {
        const parsedUrl = new URL(this.stvdioSpaceUrl);
    ...
    parsePermalink(fullUrl: string): PermalinkParts {
        if (!fullUrl || !fullUrl.startsWith(this.stvdioSpaceUrl)) {
            throw new Error("Does not appear to be a permalink");
        }

        const parts = fullUrl.substring(`${this.stvdioSpaceUrl}/#/`.length);
        return StvdioSpacePermalinkConstructor.parseAppRoute(parts);
    }
